...
public class GetLaitNamesTask extends AsyncTask<Void,Void,Void> {
    ...
    public GetLaitNamesTask(MainActivity activity, UserProductInfo userProductInfo, RecyclerView recyclerView, ProgressBar waiter,
                            ExpandableLayout expandableLayout){
        ...
        StringRequest detergentRequest = new StringRequest(StringRequest.Method.POST, detergentUrl, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    userProductInfo.laitNames.clear();
                    userProductInfo.laitIds.clear();
                    for(int i=0; i < jsonArray.length(); i++){
                        userProductInfo.laitNames.add(jsonArray.getJSONObject(i).getString("libelle"));
                        userProductInfo.laitIds.add(jsonArray.getJSONObject(i).getString("id"));
                        // lait recycler
                        LaitProductAdapter laitProductAdapter = new LaitProductAdapter(userProductInfo.getContext(),userProductInfo.laitNames,userProductInfo);
                        recyclerView.setAdapter(laitProductAdapter);
